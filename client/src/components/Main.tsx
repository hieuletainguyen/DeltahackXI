...
export default function Main() {
        ...
        if (execution.includes('Execution: ')) {
            ...
            command.forEach(cmd => {
                if (cmd.includes('setStation')) {
                    ...
                    const station = stations.find(station => station.name.toLowerCase() === stationName.toLowerCase());
                    if (station) {
            ...
            .then(data => {
                if (data.user_data) {
                    const user = {
                        ...
                        isProvider: data.user_data.isProvider
                    };
                    setUser(user);
    ...
    const handleSubmitForm = async () => {
        ...
        const bookingData = {
            ...
            username: user?.email,
            planned_time: `${startTime.hours}:${startTime.minutes} - ${endTime.hours}:${endTime.minutes}`,
            voltage: voltage
        };
        ...
        let response: BookingApiResponse | undefined; // Explicitly define the type

        if (token) {
            const apiResponse = await fetch('http://localhost:9897/api/confirm-booking', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            response = await apiResponse.json(); // Assign the response to the variable
        ...
        if (response && response.success) {
            ...
            }, 6000); // Turn off webcam after 1 minute
        }
    };
